...
template <typename T>
Chain<T>::Iterator::Iterator(ChainNode<T> *theNode) {
    this->node = theNode;
}

template <typename T>
T &Chain<T>::Iterator::operator*() const {
    return node->element;
}
...
template <typename T>
typename Chain<T>::Iterator Chain<T>::Iterator::operator++(int) {
    Iterator old = *this;
    node = node->next;
    return old;
}
...
template <typename T>
bool Chain<T>::Iterator::operator!=(const Iterator &rh) const {
    return node != rh.node;
}

template <typename T>
bool Chain<T>::Iterator::operator==(const Iterator &rh) const {
    return !(this->node != rh.node);
}
